...
def rep_main(file_name, datasheet, recon_type):
    ...
    accuracy_head = np.genfromtxt('{}/for_report/eval_box_5_accuracy_head.txt'.format(datasheet['output']),
                                  dtype=str, delimiter='    ')
    ...
    y_position = add_text(pdf, 'Body phantom:', margin, y_position+3, 'abc_b', ts1, char_per_line)
    accuracy_body = np.genfromtxt('{}/for_report/eval_box_5_accuracy_body.txt'.format(datasheet['output']),
                                  dtype=str, delimiter='    ')
